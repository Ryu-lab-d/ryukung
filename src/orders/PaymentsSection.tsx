      ...
      >
        ...
        {balanceDue > 0 && (
          <button
            ...
            className={
              'w-full rounded-lg font-semibold py-2.5 text-sm ' +
              (amount === String(balanceDue) ? 'bg-stone-900 text-white' : 'border-2 border-stone-900 text-stone-900')
            }
          >
            ✅ รับมาพอดี {formatBaht(balanceDue)} บาท
          </button>
        )}

        <div className="space-y-1">
          <label htmlFor="payment-amount" className="text-sm text-stone-600">หรือใส่จำนวนเงินเอง</label>
          <input
            id="payment-amount"
            type="number"
            step="0.01"
