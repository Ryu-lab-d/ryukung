-- ให้ลูกค้าที่สั่งเองจากหน้าเมนู (/menu) ตามดูออเดอร์ของตัวเองที่หน้า /o/:token ได้ทันทีหลังส่งคำสั่งซื้อ แม้ร้าน
-- ยังไม่ได้กดยืนยัน (is_draft=true อยู่) — เดิม get_public_order บังคับ is_draft=false เท่านั้น ทำให้ลูกค้าที่เพิ่ง
-- สั่งเองเจอ "ไม่พบออเดอร์" ทั้งที่ส่งสำเร็จจริง จึงเปิดช่องพิเศษเฉพาะ order_source='customer' เท่านั้น (ไม่เปิดให้
-- ออเดอร์ร่างของพนักงาน order_source='staff' หลุดออกไปด้วย เพราะ public_token ถูกสุ่มให้ทุกแถวตั้งแต่สร้าง
-- รวมถึงร่างพนักงานที่ยังทำงานไม่เสร็จ ซึ่งไม่ควรดูได้จากภายนอกเด็ดขาด)
...
begin
  ...
  select jsonb_build_object(
    ...
    'note',                   v_order.note,
    -- ออเดอร์รอยืนยัน (is_draft=true) ยังแก้ที่อยู่เองผ่าน update_public_order_address ไม่ได้ (ฟังก์ชันนั้นบังคับ
    -- is_draft=false) กันโชว์ปุ่มแก้ที่อยู่ทั้งที่กดแล้วจะ error เปล่าๆ
